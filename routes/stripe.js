...
router.post("/create-checkout-session", async (req, res) => {
  ...
  const line_items = req.body.cartItems.map((item) => {
    return {
      price_data: {
        ...
        unit_amount: item.price ? Math.round(item.price * 100) : 0, // Handle edge cases
      },
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: "https://mobileapp-production-8fde.up.railway.app/stripe/checkout-success",
    cancel_url:  "https://mobileapp-production-8fde.up.railway.app/stripe/cancel",
  });
